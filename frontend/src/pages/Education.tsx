...
const Education = () => {
	...
	const handleButtonClick = (exercise: any) => {
		if (exercise.category == "taskVideo1" || exercise.category == "taskVideo2") {
			navigate(`/video-practice?category=${exercise.category}`);
		} else if (exercise.category == "taskGame1" || exercise.category == "taskGame2") {
			navigate(`/game-practice?category=${exercise.category}`);
		} else if (exercise.category == "taskWriting1" || exercise.category == "taskWriting2") {
			navigate(`/writing-practice?category=${exercise.category}`);
		} else if (exercise.category == "taskVerbal" || exercise.category == "taskMath") {
			console.log("hello");
		}
	};
	return (
				...
				{exercises.practiceDescription.map((exercise) => (
					...
					>
						<div className="flex flex-row gap-2 xs:gap-4 ss:gap-6">
							<p className="text-smrtBlue font-black text-5xl">{exercise.date}</p>
							<div className="leading-6">
								<p className="text-lg ss:text-xl text-smrtBlack">{exercise.month}</p>
								<p className="font-medium text-practiceGray">{exercise.time}</p>
							</div>

							<div className="ml-4">
								<p className="text-base ss:text-xl text-smrtBlack">{exercise.title}</p>
								<p className="font-medium text-practiceGray">{exercise.task}</p>
							</div>
						</div>
						<button
